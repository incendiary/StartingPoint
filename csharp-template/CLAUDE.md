...
```bash
# Working tree
grep -rniE "(key|secret|token|password|auth|bearer|connectionstring|apikey)" . \
  --include="*.cs" --include="*.csproj" --include="*.json" --include="*.xml" \
  --include="*.config" --include="*.yaml" --include="*.yml"

# Git history
git log -p | grep -iE "(key|secret|token|password|bearer|connectionstring)" | head -100
```
...
- Review `*.csproj` NuGet dependencies for severely outdated or CVE-affected packages.
- Run `dotnet list package --vulnerable` if .NET 6+.
...
Review every `.cs` file:
- **Dead code** — unreachable branches, unused usings, unused private members
- **Duplication** — repeated logic that should be extracted
- **Confusing control flow** — catch-and-re-throw without adding context, empty catch blocks, dead else branches
- **Comment noise** — XML doc comments that restate the method name, inline comments that describe *what* not *why*
...
```bash
# Install dotnet format (built-in .NET 6+, otherwise:)
